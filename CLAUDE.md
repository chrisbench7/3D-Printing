...
Spec + Phase 0 test plates. **Now: Phase 0 spike** — test plates and procedure in [`phase0/`](phase0/README.md), generated by `node tools/phase0/generate.mjs` from [`config/printers.json`](config/printers.json). Phase 0 checks (does a nested plate survive import into SprintRay RayWare, measured printer profiles, packing baseline vs. RayWare's own auto-arrange). Nothing after Phase 0 gets built until its gate passes.

| Phase | Scope | Status |
|---|---|---|
| 0 | RayWare handoff spike + printer profiles | 🟡 Test plates built; awaiting RayWare results |
